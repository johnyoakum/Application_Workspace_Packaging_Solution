...
.PRE-REQUISITES
    ...
    -Have ran and populated Log Analytics with Installed Applications

.DESCRIPTION
    This script will query Log Analytics for the data that was put there through the previous script of inventory of installed software.
    ...
    Thinking about adding the ability to even assign entitlements, but that might just wait.

.EXAMPLE
    .\Create-AWApplications.ps1 
...
$WorkspaceID = "WorkspaceID" # Replace this with you own
...
$password = 'apipassword' # Enter the password for that service Account
$ClosingTime = 15
...
| summarize by Name_s, Publisher_s, Version_s, GUID_g, IsDeleted_b
| where IsDeleted_b == false
| project Name_s, Publisher_s, Version_s, GUID_g
| order by Name_s asc "

#region Second Form
# Code for main Application Picker
[xml]$AWPicker = @"
<Window 
        Name="Main"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Application Workspace Package Utility" Height="800" Width="1200" ResizeMode="NoResize" Background="#FF323F48" FontFamily="Work Sans">
    <Grid>
        <Image Name="Image" HorizontalAlignment="Center" Height="107" Margin="0,59,0,0" VerticalAlignment="Top" Width="1039" />
        <TextBlock HorizontalAlignment="Center" Height="90" Margin="0,184,0,0" TextAlignment="Center" TextWrapping="Wrap" FontFamily="Work Sans" Text="Listed below are the apps that you have installed in your environment that have a possible match to an application that is available via the Application Workspace Setup Store. Here you can select the applications you want to enable and then have it bring them into your environment." VerticalAlignment="Top" Width="665" Foreground="White" FontSize="16"/>
        <ListView Name="ListView" Margin="58,287,46,107" FontFamily="Work Sans" Background="#FFE0E4E1" SelectionMode="Multiple">
            <ListView.View>
                <GridView>
                    ...
                    <GridViewColumn Header="Application Name" DisplayMemberBinding="{Binding AWName}" Width="250"/>
                    <GridViewColumn Header="Version" DisplayMemberBinding="{Binding AWVersion}" Width="90"/>
                    ...
                    <GridViewColumn Header="Exists" Width="50">
                        <GridViewColumn.CellTemplate>
                            <DataTemplate>
                                <Border HorizontalAlignment="Center" VerticalAlignment="Center">
                                    <CheckBox IsChecked="{Binding Exists}" IsEnabled="False" />
                                </Border>
                            </DataTemplate>
                        </GridViewColumn.CellTemplate>
                    </GridViewColumn>

                    <GridViewColumn Header="Matched To" DisplayMemberBinding="{Binding NameSearched}" Width="115"/>
                    <GridViewColumn Header="Installed Application" DisplayMemberBinding="{Binding OriginalApplication}" Width="250"/>
                    <GridViewColumn Header="Application GUID" DisplayMemberBinding="{Binding AWID}"/>
                </GridView>
            </ListView.View>
        </ListView>
        ...
        <TextBox Name="ClosingWarning" Background="#FF323F48" FontFamily="Work Sans" TextAlignment="Center" HorizontalAlignment="Left" Height="28" Margin="80,714,0,0" TextWrapping="Wrap" Text="Exiting in 30 seconds" VerticalAlignment="Top" Width="686" Foreground="White" FontSize="18"/>
    </Grid>
</Window>
"@
#endregion
...
<Window 
    ...
    <Grid>
        <Image Name="Image" HorizontalAlignment="Center" Height="107" Margin="0,59,0,0" VerticalAlignment="Top" Width="1039" />
        <TextBlock Name="ClosingMessage" HorizontalAlignment="Center" Height="248" Margin="0,265,0,0" TextWrapping="Wrap" Text="Thank you for using this Utility. This screen will exit in 30 seconds." VerticalAlignment="Top" Width="822" FontFamily="Work Sans" FontSize="72" Foreground="White" TextAlignment="Center"/>
    </Grid>
</Window>
...
Function Get-DataFromAPI {
    ...
    if ($response) {
        ...
        foreach ($row in $rows) {
            ...
            # Normalize the Name and add as a NoteProperty
            if ($rowObject.PSObject.Properties.Match("Name_s")) {
                $normalizedName = Normalize-Name -Name $rowObject.Name_s
                $rowObject | Add-Member -MemberType NoteProperty -Name "NormalizedName" -Value $normalizedName
            }

            # Add a placeholder for version normalization if needed
            if ($rowObject.PSObject.Properties.Match("Version_s")) {
                # Assume a placeholder Normalize-Version function
                $normalizedVersion = $rowObject.Version_s # Replace with your logic for normalizing version
                $rowObject | Add-Member -MemberType NoteProperty -Name "NormalizedVersion" -Value $normalizedVersion
            }
        ...
        If ($LiquitResources.Count -gt 0) {
            ForEach ($LR in $LiquitResources) {
                ...
                $Application =  New-Object PSObject -Property @{
                    ...
                    Exists = $Exists
                    OriginalApplication = "$($app.Name_s)"
                }
...
function Normalize-Name {
    ...
    # Remove common prefixes/suffixes, trim whitespace, and handle known replacements
    $normalized = $Name.Trim() -replace "^(Microsoft|Adobe|Google)\s+", "" -replace "\s+\(.*\)$", ""
    $normalized = $normalized -replace "[^a-zA-Z0-9\s]", ""  # Remove special characters
    return $normalized
}
...
#region Main Actions
        ...
        Try {
            $LogAnalyticsData = Get-DataFromAPI
        } catch {}
        ...
        $MatchedLiquitApps = Get-LiquitApps -InstalledApplications $LogAnalyticsData

        # Add a field to store whether to create that app or not.
        ForEach ($App in $MatchedLiquitApps) {
            $app | Add-Member -NotePropertyName CreateApp -NotePropertyValue $false
        }

        $SortedApplications = $MatchedLiquitApps | Sort-Object -Property AWName
#endregion
    ...
    $warning = $XMLApplicationForm.FindName("ClosingWarning")
    # $CheckBox = $XMLApplicationForm.FindName("CreateApp")
    ...
    $XMLReader = (New-Object System.Xml.XMLNodeReader $ClosingScreen)
    $XMLApplicationForm = [Windows.Markup.XamlReader]::Load($XMLReader)

    $MainImage = $XMLApplicationForm.FindName('Image')
    $Title = $XMLApplicationForm.FindName('Main')
    $ClosingMessage = $XMLApplicationForm.FindName('ClosingMessage')

    # Add in the Image
    $MainImage.Source = $bannerImage
    $Title.Icon = $iconImage
    $ClosingMessage.Text = "Thank you for using this Utility. This screen will exit in $ClosingTime seconds."

    $XMLApplicationForm.WindowStartupLocation = [System.Windows.Forms.FormStartPosition]::CenterScreen
    $XMLApplicationForm.Topmost = $true

    $XMLApplicationForm.Show()

    Start-Sleep -Seconds $ClosingTime
