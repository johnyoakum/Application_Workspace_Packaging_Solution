...
union
select distinct arp2.publisher0, arp2.displayname0, arp2.version0
  from v_GS_ADD_REMOVE_PROGRAMS_64 arp2 
"
$UserInstalledSoftwareQuery = "SELECT Publisher0,DisplayName0,productid0 as prodid0,Version0,InstanceCount FROM (SELECT Publisher0, DisplayName0,productid0, Version0, COUNT(ResourceID) AS InstanceCount FROM v_GS_RecastSoftware_EI_UserArp0 where Version0 <> '[Not Available]' GROUP BY Publisher0, DisplayName0,productid0, Version0) AS UserInstalledApps"
...
function Normalize-Name {
    ...
    # Step 3: Remove the section in parentheses (including the parentheses themselves)
    $normalized = $normalized -replace "\s*\(.*?\)", ""

    # Step 4: Remove common prefixes (Microsoft, Adobe, Google, etc.)
    $normalized = $normalized -replace "^(Microsoft|Adobe|Google)\s+", ""

    # Step 5: Remove special characters except spaces
    $normalized = $normalized -replace "[^a-zA-Z\s]", ""

    # Step 6: Normalize whitespace (replace multiple spaces with a single space)
    ...
    If ($CSV -ne $null) {
        ...
        ForEach ($App in $IADevices) {
            ...
            $CurrentApp = New-Object PSObject -prop @{
                ...
                NormalizedName = $(Normalize-Name -Name $($App.DisplayName0))
            }
                [void]$InstalledApps.Add($CurrentApp)
        }
